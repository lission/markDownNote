...
- InnoDB为每行记录都实现了两个隐藏字段：
  ...
  - **DB_ROLL_PTR**，7字节，回滚指针，把它理解为删除版本号，数据被删除或记录为就数据是，记录当前事务ID，没有修改或删除时是空的。
...
2. 数据版本 ==trx_id < min_trx_id（未提交事务的最小 ID）== ，说明这个版本在生成 Read View 已经提交，可以访问。
3. 数据版本的 ==trx_id > max_trx_id（下一个事务 ID）== ，这个版本是生成 Read View 之后才开启的事务建立的，不能访问。
4. 数据版本的 ==trx_id 在 min_trx_id 和 max_trx_id 之间==。如果在 ==m_ids== 中，不可以访问，如果不再 ==m_ids== 中，可以访问。
...
- **Record Lock，记录锁**，**锁定一个index record(对索引进行加锁)**，记录锁从事锁定索引记录，即使表没有创建索引，InnoDB通过内置的Row ID作为聚簇索引来进行记录锁。
- **Gap Lock，间隙锁**，锁定一个索引记录区间，查询未命中index record时(无论是等值查询还是范围查询)，采用间隙锁。**间隙锁在Repeatable Read以上的隔离级别才可用。间隙锁为了解决同一事务中出现幻读的问题。**
...
- 解决**写IO**问题，当大量写需求到达mysql时，可以通过在mysql前加上消息队列相关组件，让写请求先进队列，然后再慢慢一次对mysql进行写操作，减轻mysql写请求IO

近一步可以继续通过增加集群来进行优化，比如redis集群、mq集群等。
